...
class CapabilityResolver:
    ...
    def resolve(
        ...
        :param adapter_code: ``adapters.code`` the caller wants (``"midas_gen"``).
            ``None`` means "whatever the capability declares" — used for the
            platform-owned ``midas_task`` rows, where no MIDAS adapter is
            involved (对接规范 §2.5.1).
        ...
        """
        capability = resolve_capability(tool, action, resource)
        self._require_enabled(capability)

        if (
            adapter_code is not None
            and capability.adapter_code is not None
            and adapter_code != capability.adapter_code
        ):
            raise AdapterError(
                ErrorCode.CAPABILITY_NOT_SUPPORTED,
                f"capability {capability.code!r} 属于 adapter={capability.adapter_code!r}，"
                f"调用方指定了 adapter={adapter_code!r}（V2.1 §16.2：软件/版本不支持该能力）。",
                details={
                    "capability": capability.code,
                    "capability_adapter": capability.adapter_code,
                    "requested_adapter": adapter_code,
                },
            )
